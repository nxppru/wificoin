...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    LogPrint(BCLog::BENCH, "CreateNewBlock() packages: %.2fms (%d packages, %d updated descendants), validity: %.2fms (total %.2fms) nBits: %02x\n", 0.001 * (nTime1 - nTimeStart), nPackagesSelected, nDescendantsUpdated, 0.001 * (nTime2 - nTime1), 0.001 * (nTime2 - nTimeStart), pblock->nBits);
	LogPrintf("CreateNewBlock(): block detail %s\n", pblock->ToString());
...
bool BlockAssembler::TestPackageTransactions(const CTxMemPool::setEntries& package)
{
    for (const CTxMemPool::txiter it : package) {
        if (it->GetTx().IsCoinBase() || !IsFinalTx(it->GetTx(), nHeight, nLockTimeCutoff))
            return false;
...
void static WiFicoinMiner(CWallet *pwallet)
{
    ...
    try {
        while (true) {
            ...
            LogPrintf("Running WiFicoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("WiFicoinMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("WiFicoinMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateWiFicoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WiFicoinMiner, pwallet));
}
