...
The RPM spec file provided here is for WiFicoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://wificoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[WiFicoin git repository](https://github.com/wificoin/wificoin) but are not part of
...
Currently only `Source10` is used, for the example `wificoin.conf` file.
...
in the wificoin git repository. At present only an SVG version of the WiFicoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the wificoin
...
needed when 0.12.1 is released, a proper fix is already in the WiFicoin git
master branch.
...
idea to build WiFicoin Core against a static version of that library compiled
...
The `wificoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `wificoin-server` package. At this point, most RPM based
...
The `wificoin-server` package should have SELinux support. How to properly do
...
WiFicoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/wificoin/). He has been
packaging WiFicoin for Fedora at least since 2012.
...
4. I renamed the `wificoin` package that contains the Qt GUI to `wificoin-core` as
that appears to be how the general population refers to it, in contrast to
`wificoin-xt` or `wificoin-classic`. I wanted to make sure the general population
