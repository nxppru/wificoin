...
###   wificoincore.org and wificoin.org and compares them.
...
WORKINGDIR="/tmp/wificoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://wificoincore.org"
HOST2="https://wificoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="wificoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'wificoin-',
...
if [ $? -ne 0 ]; then
   echo "wificoin.org failed to provide signature file, but wificoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "wificoin.org and wificoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the WiFicoin Core binary release signing key installed?"
   fi
