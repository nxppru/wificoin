...
#if defined(NDEBUG)
# error "WiFicoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "WiFicoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s WFC additional fees, %d delta bytes\n",
        ...
        // Added by zhangzf 20180209. Add memory address index
        if (fAddressIndex) {
            pool.addAddressIndex(entry, view);
        }

        // Added by zhangzf 20180209. Add memory spent index
        if (fSpentIndex) {
...
static DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (fAddressIndex) {
        if (!pblocktree->EraseAddressIndex(addressIndex)) {
            AbortNode("EraseAddressIndex.", "Failed to delete address index");
        }
        if (!pblocktree->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            AbortNode("UpdateAddressUnspentIndex.", "Failed to write address unspent index");
        }
    }
...
void ThreadScriptCheck() {
    RenameThread("wificoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    {
        const CTransaction &tx = *(block.vtx[i]);
        const uint256 txhash = tx.GetHash(); //Added by zhangzf 20180208
